...
export function DfxPageHead({ title, description, actions, className, containerClassName, titleClassName, descriptionClassName, actionClassName }: iDfxPageHead) {
  return (
    <div className={cn("lg:flex lg:items-center lg:justify-between", className)}>
      <div className={cn("min-w-0 flex-1", containerClassName)}>
        ...
        <p className={cn("mt-3 text-lg text-gray-800 dark:text-gray-400", descriptionClassName)}>
          {description}
        </p>
      </div>
      <div className={cn("mt-5 flex lg:ml-4 lg:mt-0", actionClassName)}>{actions}</div>
    </div>
  );
}
